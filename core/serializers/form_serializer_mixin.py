...
class FormSerializerMixin(FieldsSerializerMixin):
    ...
    def validate(self, data):
        ...
        if not self.form_instance.is_valid():
            raise serializers.ValidationError({'errors': self.form_instance.errors})

        return self.form_instance.cleaned_data
    ...
    def get_form(self, data=None, files=None, *args, **kwargs):
        ...
        assert hasattr(self, 'Meta'), (
            'Class {form_serializer_class} missing "Meta" class'.format(
                form_serializer_class=self.__class__.__name__
            )
        )

        assert hasattr(self.Meta, 'form'), (
            'Class {form_serializer_class} missing "Meta.form" attribute'.format(
                form_serializer_class=self.__class__.__name__
            )
        )
        ...
        if not self.form_instance:
            self.form_instance = self.Meta.form(data=data, files=files, *args, **kwargs)
